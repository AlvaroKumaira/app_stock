...
faturamento = """
        SELECT
            ...
            AND MONTH(CONVERT(DATETIME, STUFF(STUFF(CAST(SD2.D2_EMISSAO AS VARCHAR), 7, 0, '-'), 5, 0, '-'))) = MONTH(GETDATE())
...
quantidade_receber = """
        SELECT
            ...
            INNER JOIN
                SB1010 AS SB ON SC7.C7_PRODUTO = SB.B1_COD
            WHERE SC7.D_E_L_E_T_ <> '*'
            AND DAY(CONVERT(DATETIME, STUFF(STUFF(CAST(SC7.C7_DATPRF AS VARCHAR), 7, 0, '-'), 5, 0, '-'))) >= 1
            AND MONTH(CONVERT(DATETIME, STUFF(STUFF(CAST(SC7.C7_EMISSAO AS VARCHAR), 7, 0, '-'), 5, 0, '-'))) >= (MONTH(GETDATE()) - 4)
            AND YEAR(CONVERT(DATETIME, STUFF(STUFF(CAST(SC7.C7_EMISSAO AS VARCHAR), 7, 0, '-'), 5, 0, '-'))) = YEAR(GETDATE())
...
FROM
    SB1010 AS P
INNER JOIN
    SB2010 AS S ON RTRIM(P.B1_COD) = RTRIM(S.B2_COD) AND S.B2_FILIAL = '0101' AND S.B2_LOCAL = 'A01' AND S.D_E_L_E_T_ <> '*'
WHERE
    P.D_E_L_E_T_ <> '*' AND
    P.B1_GRUPO NOT IN ('002', '001', '003') AND
    P.B1_TIPO IN ('ME', 'MI', 'KT', 'PA') AND
