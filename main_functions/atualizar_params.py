...
def merge_sheets(excel_path):
    ...
    # Merge all three dataframes on cod_agrup using outer join
    merged_df = df1.merge(df2, on=['B1_ZGRUPO', 'Descrição'], how='left')
    merged_df = merged_df.merge(df3, on=['B1_ZGRUPO', 'Descrição'], how='left')
    merged_df.dropna(subset=['B1_ZGRUPO'], inplace=True)

    # If TempN_Comprar is NaN, it means cod_agrup is not from df3
    ...
    merged_df.fillna({"Descrição": "", "mult": 1, "seguranca": 0, "n_comprar": 0}, inplace=True)
    print(merged_df[merged_df['B1_ZGRUPO'] == '81852589'])
